...
export default function App() {
  ...
  async function handleMatch() {
    ...
    const brideDob =
      bride.date && bride.time ? `${bride.date}T${bride.time}:00` : "";
    const groomDob =
      groom.date && groom.time ? `${groom.date}T${groom.time}:00` : "";
    ...
    try {
      const res = await fetch("http://localhost:4000/match", {
        ...
        body: JSON.stringify({
          bride: { ...bride, dob: brideDob },
          groom: { ...groom, dob: groomDob },
        }),
      });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-4 border rounded shadow">
          ...
          <input
            type="date"
            value={bride.date}
            onChange={(e) => setBride({ ...bride, date: e.target.value })}
            className="w-full border rounded p-2 mb-2"
          />
          <input
            type="time"
            value={bride.time}
            onChange={(e) => setBride({ ...bride, time: e.target.value })}
            className="w-full border rounded p-2 mb-2"
          />
          <LocationPicker
            label="Birth Location"
            value={bride.location}
            onChange={(coords) =>
              setBride({ ...bride, lat: coords.lat, lon: coords.lng })
            }
          />
        </div>
        ...
        <div className="p-4 border rounded shadow">
          ...
          <input
            type="date"
            value={groom.date}
            onChange={(e) => setGroom({ ...groom, date: e.target.value })}
            className="w-full border rounded p-2 mb-2"
          />
          <input
            type="time"
            value={groom.time}
            onChange={(e) => setGroom({ ...groom, time: e.target.value })}
            className="w-full border rounded p-2 mb-2"
          />
          <LocationPicker
            label="Birth Location"
            value={groom.location}
            onChange={(coords) =>
              setGroom({ ...groom, lat: coords.lat, lon: coords.lng })
            }
          />
        </div>
      </div>
      ...
      {result && (
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-4 border rounded shadow bg-white">
            ...
            <p>
              <strong>Name:</strong> {bride.name}
            </p>
            <p>
              <strong>Rashi:</strong> {result.bride?.rashi}
            </p>
            <p>
              <strong>Nakshatra:</strong> {result.bride?.nakshatra} (Pada{" "}
              {result.bride?.pada})
            </p>
          ...
          <div className="p-4 border rounded shadow bg-white">
            ...
            <p>
              <strong>Name:</strong> {groom.name}
            </p>
            <p>
              <strong>Rashi:</strong> {result.groom?.rashi}
            </p>
            <p>
              <strong>Nakshatra:</strong> {result.groom?.nakshatra} (Pada{" "}
              {result.groom?.pada})
            </p>
            <h3 className="mt-3 font-semibold">Grahas</h3>
            <ul className="list-disc ml-5 text-sm">
